 ...
 */
if ( class_exists( 'Mwb_Gf_Integration_With_Zoho_CRM_Onboarding' ) ) {
	return;
}
 ...
 */
class Mwb_Gf_Integration_With_Zoho_CRM_Onboarding {
	...
	public function enqueue_styles() {
		...
		if ( $this->is_valid_page_screen() ) {
			...
			wp_enqueue_style( 'makewebbetter-onboarding-style', ZOHO_GF_INTEGRATION_URL . 'admin/css/makewebbetter-onboarding-admin.css', array(), '1.0.0', 'all' );

			// Uncomment Only when your plugin doesn't uses the Select2.
			wp_enqueue_style( 'makewebbetter-onboarding-select2-style', ZOHO_GF_INTEGRATION_URL . 'admin/css/select2.min.css', array(), '1.0.0', 'all' );
		}
	}
	...
	public function enqueue_scripts() {
		...
		if ( $this->is_valid_page_screen() ) {
			...
			wp_enqueue_script( 'makewebbetter-onboarding-scripts', ZOHO_GF_INTEGRATION_URL . 'admin/js/makewebbetter-onboarding-admin.js', array( 'jquery' ), '1.0.0', true );
			...
			// Uncomment Only when your plugin doesn't uses the Select2.
			wp_enqueue_script( 'makewebbetter-onboarding-select2-script', ZOHO_GF_INTEGRATION_URL . 'admin/js/select2.min.js', array( 'jquery' ), '1.0.0', false );
		}
	}
	...
	public function add_onboarding_popup_screen() {
		...
		if ( $this->is_valid_page_screen() && $this->can_show_onboarding_popup() ) {
			require_once ZOHO_GF_INTEGRATION_DIRPATH . 'extra-templates/makewebbetter-onboarding-template-display.php';
		}
	}
	...
	public function add_deactivation_popup_screen() {
		...
		if ( ! empty( $pagenow ) && 'plugins.php' == $pagenow ) { // @codingStandardsIgnoreLine
			require_once ZOHO_GF_INTEGRATION_DIRPATH . 'extra-templates/makewebbetter-deactivation-template-display.php';
		}
	}
	...
	public function is_valid_page_screen() {
		...
		if ( ! empty( $screen->id ) ) {
			...
			$is_valid = in_array( $screen->id, apply_filters( 'mwb_helper_valid_frontend_screens', array() ) ) && $this->add_mwb_additional_validation(); // @codingStandardsIgnoreLine
		}

		if ( empty( $is_valid ) && 'plugins.php' == $pagenow ) { // @codingStandardsIgnoreLine
			$is_valid = true;
		}
	...
	public function can_show_onboarding_popup() {
		...
		if ( ! empty( $is_already_sent ) && 'sent' == $is_already_sent ) { // @codingStandardsIgnoreLine
			return false;
		}
	...
	public function add_on_boarding_form_fields() {
		...
		$fields = array(
			...
			wp_rand() => array(
				'id'          => 'monthly-revenue',
				'label'       => esc_html__( 'What is your monthly revenue?', 'mwb-gf-integration-with-zoho-crm' ),
				'type'        => 'radio',
			...
			wp_rand() => array(
				'id'          => 'industry_type',
				'label'       => esc_html__( 'What industry defines your business?', 'mwb-gf-integration-with-zoho-crm' ),
				'type'        => 'select',
				'name'        => 'industry_type_',
				'value'       => '',
				'multiple'    => 'yes',
			...
			wp_rand() => array(
				'id'          => 'onboard-email',
				'label'       => esc_html__( 'What is the best email address to contact you?', 'mwb-gf-integration-with-zoho-crm' ),
				'type'        => 'email',
				'name'        => 'email',
			...
			wp_rand() => array(
				'id'          => 'onboard-number',
				'label'       => esc_html__( 'What is your contact number?', 'mwb-gf-integration-with-zoho-crm' ),
				'type'        => 'text',
				'name'        => 'phone',
				'value'       => '',
				'required'    => 'yes',
	...
	public function render_field_html( $attr = array(), $base_class = 'on-boarding' ) {
		...
		$multiple = ! empty( $attr['multiple'] ) && 'yes' == $attr['multiple'] ? 'yes' : 'no'; // @codingStandardsIgnoreLine
		$required = ! empty( $attr['required'] ) ? 'required="required"' : '';

		$html = '';

		if ( 'hidden' != $type ) : // @codingStandardsIgnoreLine ?>
		...
		switch ( $type ) {
			...
			case 'radio':
				...
				if ( ! empty( $options ) && is_array( $options ) ) :
					...
					<?php
					$is_multiple = ! empty( $multiple ) && 'yes' != $multiple ? 'name = "' . $name . '"' : ''; // @codingStandardsIgnoreLine
			...
			case 'select2':
				...
				if ( ! empty( $options ) && is_array( $options ) ) {
					...
					$is_multiple = 'yes' == $multiple ? 'multiple' : ''; // @codingStandardsIgnoreLine
					$select2     = ( 'yes' == $multiple && 'select' == $type ) || 'select2' == $type ? 'on-boarding-select2 ' : ''; // @codingStandardsIgnoreLine
					?>
					...
					<select class="on-boarding-select-field <?php echo esc_html( $select2 ); ?> <?php echo esc_html( $class ); ?>" id="<?php echo esc_html( $id ); ?>" name="<?php echo esc_html( $name ); ?>[]" <?php echo esc_html( $required ); ?> <?php echo esc_html( $is_multiple ); ?>>
						...
						<?php if ( 'select' == $type ) : // @codingStandardsIgnoreLine ?>	
							<option class="on-boarding-options" value=""><?php esc_html_e( 'Select Any One Option...', 'mwb-gf-integration-with-zoho-crm' ); ?></option>
						<?php endif; ?>
	...
	public function send_onboarding_data() {
		...
		if ( ! empty( $form_data ) && is_array( $form_data ) ) {
			...
			foreach ( $form_data as $key => $input ) {
				...
				if ( 'show-counter' == $input->name ) { // @codingStandardsIgnoreLine
					continue;
				}
		...
		try {
			...
			$found = current(
				array_filter(
					$formatted_data,
					function( $item ) {
						return isset( $item['name'] ) && 'plugin_deactivation_reason' == $item['name']; // @codingStandardsIgnoreLine
					}
				)
			);
		...
		if ( ! empty( $action_type ) && 'onboarding' == $action_type ) { // @codingStandardsIgnoreLine
	...
	protected function handle_form_submission_for_hubspot( $submission = false, $action_type = 'onboarding' ) {
		...
		if ( 'onboarding' == $action_type ) { // @codingStandardsIgnoreLine
			array_push(
				$submission,
				array(
					'name'  => 'currency',
	...
	private function hic_post( $endpoint, $post_params, $headers ) {
		...
		if ( is_wp_error( $response ) ) {
			...
			$status_code = 500;
			$_response   = esc_html__( 'Unexpected Error Occured', 'mwb-gf-integration-with-zoho-crm' );
			$errors      = $response;
	...
	protected function hubwoo_submit_form( $form_data = array(), $action_type = 'onboarding' ) {
		...
		if ( 200 == $response['status_code'] ) { // @codingStandardsIgnoreLine
